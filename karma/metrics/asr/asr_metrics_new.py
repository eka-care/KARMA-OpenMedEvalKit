...
@register_metric("asr_metric_new")
class ASRMetricsNew(BaseMetric):
    ...
    def evaluate(self, predictions: List[str], references: List[str], **kwargs) -> Dict:
        ...
        if not language:
            raise ValueError("Language parameter is required. Pass it via kwargs: language='english'")
        
        try:
            print(f"Creating {language} aligner...")
            # Get language-specific aligner
            cer_threshold = kwargs.get('cer_threshold', 0.4)
            aligner = self.get_aligner(language, cer_threshold)  # FIXED: Added self.
