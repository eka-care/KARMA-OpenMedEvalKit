...
@click.option(
    "--task-type",
    help="Filter by task type (e.g., 'mcqa', 'vqa', 'translation')"
)
@click.option(
    "--metric",
    help="Filter by supported metric (e.g., 'accuracy', 'bleu')"
)
@click.option(
    "--format",
    "output_format",
...
@click.option(
    "--show-args",
    is_flag=True,
    help="Show detailed argument information"
)
@click.pass_context
    ...
    try:
        ...
        for dataset_name in all_datasets:
            try:
                ...
                console.print(ClickFormatter.warning(f"Could not get info for dataset '{dataset_name}': {e}"))
        ...
        if not filtered_datasets:
            ...
            console.print(ClickFormatter.warning(f"No datasets found matching filters: {filter_str}"))
            return
...
def _show_detailed_args(console: Console, datasets_info: dict) -> None:
    ...
    """
    datasets_with_args = {
        name: info for name, info in datasets_info.items()
        if (info.get('required_args') or info.get('optional_args') or info.get('default_args'))
    }
...
def list_all(ctx, output_format):
    ...
    console.print(Panel.fit(
        "[bold cyan]Karma Registry Overview[/bold cyan]",
        border_style="cyan"
    ))
