...
@click.option(
    "--show-code",
    is_flag=True,
    help="Show model class code location and basic info"
)
@click.pass_context
def info_model(ctx, model_name, show_code):
    ...
    try:
        ...
        if not model_registry.is_registered(model_name):
            ...
            console.print(ClickFormatter.error(f"Model '{model_name}' not found in registry"))
        ...
        if show_code:
            ...
            if file_path != 'Unknown' and Path(file_path).exists():
                ...
                console.print(f"  Line: {getattr(model_class, '__qualname__', 'Unknown')}")
            else:
...
@click.option(
    "--show-examples",
    is_flag=True,
    help="Show usage examples with arguments"
)
@click.option(
    "--show-code",
    is_flag=True,
    help="Show dataset class code location"
)
@click.pass_context
def info_dataset(ctx, dataset_name, show_examples, show_code):
    ...
    try:
        ...
        if not dataset_registry.is_registered(dataset_name):
            ...
            console.print(ClickFormatter.error(f"Dataset '{dataset_name}' not found in registry"))
        ...
        # Show usage examples
        if show_examples or dataset_info.get('required_args') or dataset_info.get('optional_args'):
...
@click.option(
    "--cache-path",
    default="./cache.db",
    help="Path to cache database to check"
)
@click.pass_context
def info_system(ctx, cache_path):
    ...
    try:
        ...
        try:
            import karma
            console.print(f"  Karma CLI: {getattr(karma, '__version__', 'development')}")
        except:
...
def _show_dataset_examples(console: Console, dataset_name: str, dataset_info: dict) -> None:
    """
    ...
    if not required_args:
        ...
        console.print(f"  karma eval --model qwen --model-path \"path/to/model\" --datasets {dataset_name}")
