...
class ResultsFormatter:
    ...
    @staticmethod
    def format_results_table(results: Dict[str, Any], title: str = "Evaluation Results") -> Table:
        """
    ...
    def format_summary_table(results: Dict[str, Any]) -> Table:
        ...
        if 'total_datasets' in summary and 'successful_datasets' in summary:
            success_rate = summary['successful_datasets'] / summary['total_datasets'] * 100
            table.add_row(
                "Datasets", 
                f"{summary['successful_datasets']}/{summary['total_datasets']} ({success_rate:.1f}%)"
            )
        if 'total_evaluation_time' in summary:
            table.add_row("Total Time", format_duration(summary['total_evaluation_time']))
