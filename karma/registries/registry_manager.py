...
class RegistryManager:
    ...
    def discover_all_registries(self, use_cache: bool = True, parallel: bool = True) -> Dict[str, RegistryDiscoveryResult]:
        """
    ...
    def _discover_parallel(self, use_cache: bool) -> Dict[str, RegistryDiscoveryResult]:
        ...
        logger.debug(f"Starting parallel discovery of {len(discovery_tasks)} registries")
        
        with ThreadPoolExecutor(max_workers=4, thread_name_prefix="registry-discovery") as executor:
            ...
            for future in as_completed(future_to_registry):
                ...
                try:
                    ...
                    logger.debug(f"Completed {registry_name} discovery: {result.discovery_time:.2f}s")
    ...
    def _discover_sequential(self, use_cache: bool) -> Dict[str, RegistryDiscoveryResult]:
        ...
        logger.debug(f"Starting sequential discovery of {len(discovery_tasks)} registries")
    ...
    def get_discovery_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "successful_discoveries": sum(1 for r in self._discovery_results.values() if r.success),
            "cache_hits": sum(1 for r in self._discovery_results.values() if r.from_cache),
            "individual_results": {
                name: {
    ...
    def invalidate_all_caches(self) -> None:
        ...
        try:
            from karma.registries.cache_manager import clear_all_registry_caches
            clear_all_registry_caches()
            logger.info("Invalidated all registry caches")
        except Exception as e:
            logger.error(f"Failed to invalidate caches: {e}")
    
    def force_rediscovery(self, parallel: bool = True) -> Dict[str, RegistryDiscoveryResult]:
        """
        Force rediscovery of all registries without using cache.
        ...
        try:
            ...
            model_registry._discovered = False
            dataset_registry._discovered = False
            metric_registry._discovered = False
            processor_registry._discovered = False
            
        except ImportError as e:
            logger.warning(f"Could not reset discovery flags: {e}")
        
        return self.discover_all_registries(use_cache=False, parallel=parallel)
...
def discover_all_registries(use_cache: bool = True, parallel: bool = True) -> Dict[str, RegistryDiscoveryResult]:
    """Convenience function to discover all registries."""
    manager = get_registry_manager()
    return manager.discover_all_registries(use_cache=use_cache, parallel=parallel)
