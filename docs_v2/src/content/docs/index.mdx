...
hero:
  ...
  actions:
    ...
    - text: View on GitHub
      ...
      variant: minimal
    - text: LLMs.txt
      link: https://karma.eka.care/llms-full.txt
      icon: external
      variant: minimal
---
...
<CardGrid>
	...
	<Card title="Extensible" icon="approve-check">
		Bring your own model, dataset or even metric. Integrated with Huggingface artifacts and supports local evaluation.
	</Card>
</CardGrid>
...
```bash
$ karma eval --model "Qwen/Qwen3-0.6B" --datasets openlifescienceai/pubmedqa --max-samples 5
